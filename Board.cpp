...
Board::Board(int rows, int columns, const map<string, int> &ships, map<string, int> alphaLookup_) {
    this->rows = rows;
    this->columns = columns;
    this->ships = ships;
    this->numberOfShips = ships.size();
    sunkedShips = 0;
    this->alphaLookup_ = alphaLookup_;
}
                    ...
                    if (noHorizontalCollision(coordinate, myShips[index].getLength())) {
                        ...
                        for (int j = 0; j < myShips[index].getLength(); j++) {
                            myGrid[rowIndex][columnIndex + j] =
                                    myShips[index].getSign();   //set tile to ship
...
//TODO: do not ask for direction when ship length is 1
        ...
        do {
            if (isValidHorizontalDirection(direction)) {
                ...
                coordinate = Helpers::getCoordinate(input, alphaLookup_, rows);
...
void Board::printShips() {
    ...
    for (int i = 0; i < numberOfShips; i++) {
        cout << i << setw(4) << "-  " << myShips[i].isPlaced() << setw(4) << "-  " << myShips[i].getName() << ","
             << myShips[i].getLength()
             << "\n";
    }
    cout << "\n\n";
